...
Class Product_model extends MY_model {
    ...
    var $column_order = array('cart_product.product_name',null); //set column field database for datatable orderable

    var $column_search = array('cart_product.product_name'); //set column field database for datatable searchable just firstname , lastname , address are searchable

    var $order = array('cart_product.id' => 'desc');


    private function get_datatables_query()
    {
        $this->db->select('cart_product.id,cart_product.product_name,cart_product.cat_id,cart_category.cat_name,cart_product.status,cart_product.meta_title,cart_product.meta_key,cart_product.canonical_link');
    ...
    public function get_details($id) {
        ...
        $this->db->where('cart_product.id',$id);
        $query = $this->db->get();
    ...
    public function insertCartProduct($value,$highlights='',$feature='') { 
        ...
        if ($this->db->insert($this->table, $value)) {
           ...
           $insert_id = $this->db->insert_id();
            ...
            {
              $value_category= array('product_id' => $insert_id,'highlights' => $highlight);
            ...
            if(!empty($feature)) {
                ...
                foreach($feature as $features) 
                {
                    ...
                    $value_feature= array('product_id' => $insert_id,'feature_id' => $features);
    ...
    public function insertCartStock($value,$multi_image='',$options='',$product_id='',$value_product='') { 

        if ($this->db->insert('cart_stock', $value)) {

            $insert_id = $this->db->insert_id();

            $this->db->update('cart_product', $value_product, array('id'=>$product_id));
            ...
            if(!empty($options[0])) {
                ...
                $this->db->where('uneek',$uniq_value);
                $query = $this->db->get('cart_stock_option');
                ...
                if ($query->num_rows() >= 1){
                    ...
                    $value_options= array('option_id' => $option,'comb_id' => $insert_id,'uneek' => $uniq_value);
                    $this->db->insert('cart_stock_option', $value_options);
                    }
    ...
    public function getAllStockImageSub($id='') {
        ...
        $this->db->select("cart_product_image.id,cart_product_image.image,cart_product_image.stock_id,cart_product_image.position");
        $this->db->from('cart_product_image');
        $this->db->where('cart_product_image.stock_id',$id);
        $this->db->order_by('cart_product_image.id','ASC');
        $query = $this->db->get();
    ...
    public function updateStock($id,$product_id,$value,$multi_image,$options) { 
        ...
        if(!empty($options[0])) {
            ...
            if ($query->num_rows() == 0) {
                ...
                $this->db->delete('cart_stock_option'); 

                foreach($options as  $index => $option) 
                {
                    $value_options= array('option_id' => $option,'comb_id' => $id,'uneek' => $uniq_value);
                    $this->db->insert('cart_stock_option', $value_options);
                }
    ...
    var $column_order2 = array('cart_product_image.id'); //set column field database for datatable orderable
    
    var $column_search2 = array('cart_product_image.id','cart_product_image.image','cart_product_image.position'); //set column field database for datatable searchable just firstname , lastname , address are searchable
    
    var $order2 = array('cart_product_image.id' => 'desc');


    private function get_datatables_query_image($stock_id='')
    {
        $this->db->select('cart_product_image.*');
        $this->db->from('cart_product_image');
        $this->db->where('cart_product_image.stock_id', $stock_id);
        $i = 0;
    ...
    public function get_datatables_image($stock_id='') {

        $this->get_datatables_query_image($stock_id);
    ...
    public function count_filtered_image($stock_id='') {
    ...
    public function count_all_image($stock_id='') {
    ...
    public function updateImageCount($stock_id='',$value='',$id='') {
        ...
        $val = array('position' => $value);

        $this->db->where('stock_id',$stock_id);
        $this->db->where('id',$id);
        $this->db->update('cart_product_image',$val);
