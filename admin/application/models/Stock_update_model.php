...
Class Stock_update_model extends MY_model {
    ...
    var $column_order = array('cart_stock.stock_name','cart_stock.price','cart_stock.list_price','cart_stock.stock',null,null,null); //set column field database for datatable orderable
    
    var $column_search = array('cart_stock.stock_name','cart_stock.price','cart_stock.list_price','cart_stock.stock'); //set column field database for datatable searchable just firstname , lastname , address are searchable
    ...
    {
        $this->db->select('cart_stock.id,cart_stock.stock_name,cart_product.cat_id,cart_stock.price,cart_stock.list_price,cart_stock.stock,cart_stock.status,cart_stock.kg,cart_stock.slug_url');
    ...
    public function UpdateKgSet($value='',$id='')
    {
      $data= array('kg' => $value);
    ...
    public function UpdateSlugSet($slug='',$id='')
    {
      ...
      $data = $query->num_rows();
        ...
        if($data < 1){
          ...
          $val = array('slug_url' => $slug);

          $this->db->where('id',$id);
          $this->db->update('cart_stock', $val);
