...
Class Cart_orders_model extends MY_model {
    ...
    {
        $this->db->select('cart_order.id,cart_order.payment_method,user_registration.phone,user_registration.prof_image,user_registration.user_name,SUM(cart_order_sub.sub_total * cart_order_sub.qty) AS total_amt,cart_order.date,cart_order_sub.status, cart_order_sub.qty as quantity,cart_order.delivery_charge,cart_order.total_amt as total');
        ...
        //  $this->db->join('cart_stock', 'cart_order_sub.stock_id = cart_stock.product_id');
        ...
        if(!empty($status )) {
                ...
                if($status == 'cancel') {
                    ...
                    $this->db->where('cart_order_sub.status',0);
                }else {
    ...
    public function viewOrderData($id) {
        $this->db->select('cart_order.id,user_registration.prof_image,user_registration.user_name,SUM(cart_order_sub.sub_total * cart_order_sub.qty) AS total_amt,cart_order.date,cart_order_sub.status,payment_id,cart_order.order_id as raz_orderid,,cart_order.delivery_charge,cart_order.total_amt as total');
        $this->db->from($this->table);
        $this->db->join('cart_order_sub', 'cart_order.id = cart_order_sub.order_id');
        $this->db->join('user_registration', 'cart_order.user_id = user_registration.id');
    ...
    {
        $this->db->select('cart_order_sub.id,cart_stock.id as stock_id,cart_stock.stock_name,cart_stock.list_price, cart_order_sub.sub_total AS subtotal,cart_order_sub.qty,cart_order_sub.status,cart_product_image.image,cart_product.description');
     ...
     public function viewOrderDeliveryAddressData($id) {
         
        $this->db->select('cart_order.id,user_delivery_address.user_id,user_delivery_address.name,user_delivery_address.address,user_delivery_address.pincode,user_delivery_address.land_mark,user_delivery_address.country,user_delivery_address.phone,user_delivery_address.alter_phone,user_delivery_address.status');
        $this->db->from($this->table);
        // $this->db->join('cart_order_sub', 'cart_order.id = cart_order_sub.order_id');
        $this->db->join('user_registration', 'cart_order.user_id = user_registration.id');
