...
class Stock_update extends MY_Auth_Controller {
    ...
    public function stock_list() {
        ...
        foreach ($list as $rows) {
	        ...
	        $row[] = $no;
	        $row[] = $rows['stock_name'];
	        $row[] = '<input style="color: '.$color.'" size="2" id="'.$rows['id'].'price" type="number" value="'.$rows['price'].'" onkeyup = "checkPrice('.$rows['id'].')" >';
	        $row[] = '<input size="2" id="'.$rows['id'].'list_price" type="number" value="'.$rows['list_price'].'" onkeyup = "checkListPrice('.$rows['id'].')" >';
	        $row[] = '<input size="2"  id="'.$rows['id'].'stock" type="number" value="'.$rows['stock'].'" onkeyup = "checkStock('.$rows['id'].')" >';
	        $row[] = '<input size="2" id="'.$rows['id'].'kg" type="number" step="0.001" value="'.$rows['kg'].'" onkeyup = "checkKg('.$rows['id'].')" >';

	        if($rows['slug_url'] !=''){

	        $row[] = '<a data-toggle="modal" data-id='.$rows['id'].' data-target="#slug-modal" class="btn  bg-olive" href="#">'.$rows['slug_url'].'</a>';

	        }
	        else{

	        $row[] = '<a data-toggle="modal" data-id='.$rows['id'].' data-target="#slug-modal" class="btn  bg-olive" href="#">Add Slug</a>';

	        }
	                
	        // $row[] = '<input type="file" name="images[]" id="file_input'.$rows['id'].'" multiple /><a data-toggle="modal" data-id='.$rows['id'].' data-target="#priceModal" class="btn btn-primary  btn-sm" ="button" href="#">Upload</a>';
    ...
    public function setKg() {

        $value=$_POST['value'];

        $id=$_POST['rowid'];

        $result=$this->model->UpdateKgSet($value,$id);
    ...
    public function slug_details() {

        $id = $this->input->post('rowid');
        
        $data['slug'] = $this->model->getSlug($id);

        $this->load->view('stock/slug',$data);

    }

    public function slug_ins() {
           
            $id = $_POST['id'];
            $slug = $_POST['slug'];
            
            $status = $this->model->UpdateSlugSet($slug,$id); 
