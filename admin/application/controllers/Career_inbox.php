<?php if (!defined('BASEPATH')) exit('No direct script access allowed');

class Career_inbox extends MY_Auth_Controller {

    protected $ci_name;//declare ci_name varriabe current controler name as image folder name to upload image

    public function __construct() 
    {
        parent::__construct();
        $this->ci_name = strtolower($this->router->fetch_class());
        $this->load->model('Career_inbox_model','model');

        if (!$this->is_logged_in()) //login only registered user from db
        { 
          redirect('Login');
        }
    }
  
    public function index() {

        $this->load->view('career-inbox/list');

    }

    public function career_list() {

        $list = $this->model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $rows) {
        $no++;
        $row = array();
        $row[] = $no;
        $row[] = $rows['name'];
        $row[] = $rows['phone'];
        $row[] = $rows['position'];

        $row[] = '<a target="_blank" href="'.CUSTOM_BASE_URL.'uploads/career/'.$rows['image_file'].'" class="btn  btn-primary">View</a>';
          
        //add html for action
        $row[] = '<a data-toggle="modal" data-id='.$rows['id'].' data-target="#del-modal" class="btn  btn-danger" href="#"><i class="fa  fa-trash-o" aria-hidden="true"></i></a>';

        $data[] = $row;
        }
        $output = array(
        "draw" => $_POST['draw'],
        "recordsTotal" => $this->model->count_all(),
        "recordsFiltered" => $this->model->count_filtered(),
        "data" => $data,
        );
        //output to json format
        echo json_encode($output);
    }
   
    public function delete() {

        $this->load->view('career-inbox/delete');
        if (isset($_POST['delete'])) 
        {
            $id=$_POST['id'];
            $this->model->delete($id);
            redirect('career_inbox');
        }
    }


}
