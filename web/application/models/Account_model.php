...
Class Account_model extends MY_Model {
    ...
    public function getCartOrderData($id='')
    {
        ...
        $this->db->select('cart_order.id,cart_order.date,cart_order.address_id,cart_order.total_amt,cart_order.delivery_charge');
    ...
    public function insertCartOrder($value='',$qty='',$stock_id='',$sub_total='') { 
        ...
        if ($this->db->insert('cart_order', $value)) {
            ...
            $value_order= array('order_id' => $insert_id,'stock_id' => $stock_id,'qty' => $qty,'sub_total' => $sub_total,'status' => 55);
    ...
    public function insertAddCartOrder($value='',$qty='',$stock_id='',$sub_total='') { 
        ...
        if ($this->db->insert('cart_order', $value)) {
            ...
            foreach ($stock_id as $key => $stock_ids) {
                ...
                $value_order= array('order_id' => $insert_id,'stock_id' => $stock_ids,'qty' => $qty[$key],'sub_total' => $sub_total[$key],'status' => 55);
    ...
    public function updateCartOrder($id='',$value='') {
   
        $this->db->where('cart_order.id',$id);
        $this->db->update('cart_order',$value);

        return true; 

    }

    public function updateCartOrderSub($order_id='') { 

            $val = array('status'=>2);
            
            $this->db->where('order_id',$order_id);
            $this->db->update('cart_order_sub',$val);
            
            $this->db->where('id',$order_id);
            $this->db->update('cart_order',$val);
    ...
    public function getDeliveryAddressData($id='')
    {
        ...
        $this->db->select('user_delivery_address.id,user_delivery_address.name,user_delivery_address.pincode,user_delivery_address.address,user_delivery_address.land_mark,user_delivery_address.phone,user_delivery_address.status');
        $this->db->from('user_delivery_address');
    ...
    public function getWishListData($id='')
    {
        ...
        $this->db->select('cart_movetowishlist.move_id as id,cart_stock.stock_name,cart_stock.list_price,cart_movetowishlist.stock_id,cart_movetowishlist.date,cart_stock.slug_url');
    ...
    public function getMyorderData($id='')
    {
        ...
        $this->db->select('cart_order.id,cart_order_sub.id as sub_id,cart_stock.id as stock_id,cart_stock.stock_name,cart_stock.list_price,cart_order.date,cart_order_sub.status,cart_stock.slug_url');
        $this->db->from('cart_order');
        $this->db->join('cart_order_sub', 'cart_order.id = cart_order_sub.order_id');
        $this->db->join('cart_stock', 'cart_order_sub.stock_id = cart_stock.id');
        $this->db->where('cart_order.user_id',$id);
        $this->db->where('cart_order_sub.status !=',55);
    ...
    public function getMyAppoinmentData($id=''){
        ...
        $this->db->from('consulting_details');

        $this->db->where('user_id',$id);
        $this->db->order_by('consulting_details.consulting_id','DESC');
        $parent = $this->db->get();
    ...
    public function getUserAddress($id='')
    {
        ...
        foreach($categories as $val){
            ...
            $this->db->where('from_pincode',$val['pincode']);
            $query = $this->db->get();
            $dat = $query->num_rows();
            ...
            if($dat == 0){
                ...
                $this->db->where('pincode',$val['pincode']);
                $this->db->update('user_delivery_address',$value);
    ...
    public function getAllCartDataByUser($id='') {
        ...
        $this->db->select('cart_stock.id,cart_stock.stock_name,cart_stock.price,cart_stock.list_price,cart_stock.stock,cart_stock.discount,cart_addtocart.qty,cart_stock.slug_url');
        $this->db->from('cart_product');
        $this->db->join('cart_stock', 'cart_product.id = cart_stock.product_id');
        $this->db->join('cart_addtocart', 'cart_stock.id = cart_addtocart.stock_id');
        $this->db->where('cart_addtocart.user_id',$id);
        ...
        foreach($categories as $p_cat) {
            ...
            $sub_total = $this->getAllCartDataByUserSub($id,$p_cat['qty']);
            
            $categories[$i]['sub_total'] = $sub_total;

            $categories[$i]['image'] = $this->getImageDataSub($p_cat['id']);
            
            $delivery_charge = $this->getDeliveryCharge();
            
            $categories[$i]['total'] = $sub_total - $delivery_charge;
           //  $categories[$i]['total'] = $sub_total;
    ...
    public function getAllCartDataByUserSub($id='',$qty='')
    {
        ...
        if ($query->num_rows() >= 1){ 
            ...
            $sub_total = $query->result_array()[0]['sub_total'];
    ...
    public function getDeliveryAddress($id,$user_id){
    ...
    public function updatetDeliveryAddress($id,$value,$user_id){
    ...
    public function checkPincode($id){
        ...
        $this->db->select('id');
        $this->db->from('delivery_charge');
        $this->db->where('from_pincode',$dat->pincode);
        $query = $this->db->get();
    ...
    public function delete_address($id,$user_id)
    {
      ...
      $this->db->where('id',$id);
      $this->db->where('user_id',$user_id);
      $this->db->delete('user_delivery_address');
    ...
    public function getDeliveryAddressId($user_id){
    ...
    public function getDeliveryAmount($address_id='',$user_id='')
    {
        $this->db->select('user_delivery_address.pincode');
        $this->db->from('user_delivery_address');
        $this->db->where('user_delivery_address.user_id',$user_id);
        $this->db->where('user_delivery_address.id',$address_id);
        $query = $this->db->get();
        ...
        $this->db->from('delivery_charge');
        $this->db->where('from_pincode',$pincode);
        $query = $this->db->get();

        $pinDetails = $query->row();

        $pinAmount = $pinDetails->amount;

        $pincode_id = $pinDetails->id;


        $this->db->select('cart_stock.kg,cart_addtocart.qty');
        $this->db->from('cart_product');
        $this->db->join('cart_stock', 'cart_product.id = cart_stock.product_id');
        $this->db->join('cart_addtocart', 'cart_stock.id = cart_addtocart.stock_id');
        $this->db->where('cart_addtocart.user_id',$user_id);
        ...
        $this->db->where("$kg BETWEEN from_kg AND to_kg");
        $query = $this->db->get();

        $kgAmount = $query->row();

        $kgAmount = $kgAmount->amount;
    ...
    public function insertCancel($value,$order_id) {
        ...
        $this->db->insert('cart_order_cancel',$value);
    ...
    public function getDeliveryAmountSingle($address_id='',$user_id='',$product_id='')
    {
        $this->db->select('user_delivery_address.pincode');
        $this->db->from('user_delivery_address');
        $this->db->where('user_delivery_address.user_id',$user_id);
        ...
        $this->db->select('cart_stock.kg');
        $this->db->from('cart_product');
        $this->db->join('cart_stock', 'cart_product.id = cart_stock.product_id');
        $this->db->where('cart_stock.id',$product_id);
        $query = $this->db->get();
