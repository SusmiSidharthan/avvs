...
class Main_Model extends MY_Model {
    ...
    public function getGalleryImages($album_id='') {

        $this->db->select('gallery_images.image,gallery.title');
        $this->db->from('gallery_images');
        $this->db->where('album.id',$album_id);
        $this->db->join('gallery','gallery.id = gallery_images.gallery_id');
        $this->db->join('album','album.id = gallery.album_id');
        $query = $this->db->get();
    ...
    public function bookappoinment($value){
        $this->db->insert('consulting_details', $value);

        return true;
    }
    
    
     public function addSmsDetails($value){
        $this->db->insert('sms', $value);

        return true;
    }
    
    
      public function insertMsgDetails($value){
        if($this->db->insert('msg_via_contact',$value)) {
    ...
    public function getMainCategorys() {
     ...
     public function getWomanCategorys() {
   
        $this->db->select('cart_category.cat_id,cart_category.cat_name,cart_category.image as img_name,cart_category.cat_icon');
        $this->db->from('cart_category');
       // $this->db->where('cart_category.parent_id',34);
        $this->db->order_by('cart_category.cart_page_order','ASC');
        $this->db->limit(10);
        $parent = $this->db->get();
    ...
    public function getKidsCategorys(){
        
        $this->db->select('cart_category.cat_id,cart_category.parent_id,cart_category.cat_name,cart_category.image as img_name,cart_category.cat_icon');
        $this->db->from('cart_category');
        $this->db->where('cart_category.parent_id',35);
        $this->db->order_by('cart_category.cart_page_order','ASC');
        $this->db->limit(10);
        $parent = $this->db->get();
...
//var_dump($query->result());  die;
    ...
    public function getProductFilterCategoryList($cat_id='',$min_price='',$max_price='',$st_name='',$parent_id='')
    {
        ...
        if($parent_id){
            ...
            $cond2 = "AND cart_category.parent_id = $parent_id";
...
print_r($search);exit;
    ...
    public function insertGetInTouch($value) {
        

        
        if($this->db->insert('get_in_touch', $value)){
   ...
   public function getSlotDetails($doctor_id='',$date=''){

    $today = date('Y-m-d');
    $time = date('h:i a');

    $this->db->select("slot.id,DATE_FORMAT(from_time,'%h:%i %p') AS from_time,DATE_FORMAT(to_time,'%h:%i %p') AS to_time");
    $this->db->from('slot');
    $this->db->join('doctors_slot','slot.id = doctors_slot.slot_id');
    $this->db->where('doctors_slot.doctor_id',$doctor_id);
    if($today == $date){

    $this->db->where('slot.from_time >',$time);
    $this->db->where('slot.to_time >',$time);
   ...
   public function getSlotData($slot_id=''){

    $this->db->select("DATE_FORMAT(from_time,'%h:%i %p') AS from_time,DATE_FORMAT(to_time,'%h:%i %p') AS to_time");
    $this->db->from('slot');
    ...
    public function insertKeyEmail($value,$phone,$key_word,$time) { 
    ...
    public function checkOtpExp($key,$phone,$time)
    {
