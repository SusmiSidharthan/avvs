...
class Product_list extends MY_Controller {
    ...
    public function __construct() {
        ...
        if(!empty($this->userDetails)) { 
          ...
          //  var_dump($this->userDetails->id); die;
    ...
    public function index() {
        ...
        if(!empty($this->uri->segment(2))) {
            ...
            $data['meta'] = $this->model->getcategoryinfo($cat_id);
        ...
        // var_dump( $this->model->getMainCategorys()); die;
        
        $data['main_category'] = $this->model->getMainCategorys();
        
        // var_dump($this->model->getProductFilterPriceList($cat_id,$search)); die;

        $data['cart_count'] = $this->model->getCartCount($this->user_id);

        $price_range = $this->model->getProductFilterPriceList($cat_id,$search);
        
         //  var_dump(  $this->model->getProductFilterCategoryList($cat_id,$search) ); die;
    ...
    public function fetch_data() {
        ...
        $config = array();
        $config['base_url'] = '#';
        $config['total_rows'] = $this->model->count_all($minimum_price, $maximum_price, $feature, $option,$cat_id,$availability,$pop,$high,$low,$first,$search,$discount);
        $config['per_page'] = 16;
        ...
        $output = array(
         ...
         'product_list'   => $this->model->fetch_data($config["per_page"], $start, $minimum_price, $maximum_price, $feature, $option,$cat_id,$availability,$pop,$high,$low,$first,$search,$discount),
         'product_count'   => "Found ". $config['total_rows']." Products"
        );
